  ...
  '1d': [
    {
      ...
      badge: 'Aanbevolen',
      badgeColor: 'bg-oaec-success/15 text-oaec-success',
    ...
    },
    {
      ...
      badge: 'Traag',
      badgeColor: 'bg-oaec-warning/15 text-oaec-warning',
      requiresBackend: true
    },
    {
      id: 'ffd',
      ...
      badge: 'Snel',
      badgeColor: 'bg-oaec-accent/15 text-oaec-accent'
    }
  ],
  '2d': [
    {
    ...
    },
    {
      ...
      badge: 'Nieuw',
      badgeColor: 'bg-oaec-info/15 text-oaec-info'
    },
    {
      id: 'nfp',
      name: 'NFP Nesting',
      ...
      badge: 'Planned',
      badgeColor: 'bg-oaec-bg text-oaec-text-muted',
      disabled: true,
      requiresBackend: true
    }
  ]
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="relative w-full bg-oaec-bg-input rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-pointer focus:outline-none focus:ring-1 focus:ring-oaec-accent"
        style={{ border: '1px solid rgba(217, 119, 6, 0.25)' }}
      >
        <span className="flex items-center">
          <span className="block truncate text-oaec-text">{selectedAlgo.name}</span>
          {selectedAlgo.badge && (
        ...
        <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
          <svg className="h-5 w-5 text-oaec-text-muted" viewBox="0 0 20 20" fill="currentColor">
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-10 mt-1 w-full bg-oaec-bg-lighter shadow-lg max-h-60 rounded-md py-1 text-base overflow-auto focus:outline-none" style={{ border: '1px solid rgba(217, 119, 6, 0.2)' }}>
          {algorithms.map((algo) => (
            <div
              key={algo.id}
              ...
              className={`
                ...
                ${algo.disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-oaec-accent/10'}
                ${algo.id === value ? 'bg-oaec-accent/15' : ''}
              `}
            >
              <div className="flex items-center">
                <span className="font-medium block truncate text-oaec-text">{algo.name}</span>
                {algo.badge && (
              ...
              <span className="text-oaec-text-muted text-sm block truncate">
                {algo.description}
              </span>
              {algo.requiresBackend && !backendAvailable && (
                <span className="text-oaec-warning text-xs">
              ...
              {algo.id === value && (
                <span className="absolute inset-y-0 right-0 flex items-center pr-4 text-oaec-accent">
